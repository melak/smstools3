...
void make_pdu(char* number, char* message, int messagelen, int alphabet, int flash_sms, int report, int with_udh, char* udh_data, char* mode, char* pdu)
{
  int coding;
  int flags;
  char tmp[50];
  ...
  if (strcmp(mode,"old")==0)
   ...
   sprintf(pdu,"00%02X00%02X%02X%s00%02XFF%02X",flags,numberlength,numberformat,tmp,coding,messagelen);
...
int split_type_0(char* Src_Pointer, int* alphabet, char* sendr, char* date, char* time, char* message, int with_udh, char* udh)
{
  ...
  {  // sender is numeric
    ...
    swapchars(sendr);
    if (sendr[Length+padding-1]=='F')
      sendr[Length+padding-1]=0;
  }
  ...
  *message=0;  
  ...
  if (strcmp(mode,"old")!=0)
  {
    ...
    if (Length>0)
    {
      ...
      smsc[Length+padding]=0;
      swapchars(smsc);
      if (smsc[Length+padding-1]=='F')
        smsc[Length+padding-1]=0;
    }
    Pointer=Pointer+Length+padding;
  }
